function getTotalBooksCount(books) {
  let bookCount = 0 // set a count
  for (let i = 0; i < books.length; i ++) { // loop through books
  bookCount ++;// add each book to count
  }
  return bookCount
}

function getTotalAccountsCount(accounts) {
  let accountCount = 0 // set a count
  for (let i = 0; i < accounts.length; i ++) { // loop through accounts
  accountCount ++;// add each account to count
  }
  return accountCount
}

function getBooksBorrowedCount(books) {
  return books.filter((book) => { //filter each book
    const borrowed = book.borrows.some((borrow) => { //check for books that meet borrow returned status of false
